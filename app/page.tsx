...
export default function Home() {
  ...
  const [introPosts, setIntroPosts] = useState<any[]>([]); 
  ...
  useEffect(() => {
    ...
    // === THAY ĐỔI: Tải bài viết từ Supabase thay vì localStorage ===
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="container mx-auto px-4 pb-12">
        ...
        {/* ======================================================== */}
        {/* ========= KHU VỰC BÀI VIẾT (MỚI THÊM VÀO) ============== */}
        {/* ======================================================== */}
        {introPosts.length > 0 && (
          <section className="mb-20">
             <div className="text-center mb-12">
               <span className="text-sky-600 font-bold text-sm tracking-wider uppercase bg-sky-50 px-3 py-1 rounded-full">Góc Kỹ Thuật</span>
               <h2 className="text-3xl font-black text-slate-800 uppercase mt-2">CÔNG NGHỆ & HOẠT ĐỘNG</h2>
            ...
            <div className="space-y-16">
              {introPosts.map((post, index) => (
                <div 
                  key={post.id} 
                  // Logic So-Le: Nếu index là số lẻ (1,3,5...) thì đảo ngược vị trí (flex-row-reverse)
                  className={`flex flex-col md:flex-row items-center gap-8 md:gap-14 ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}
                >
                  {/* Cột Ảnh */}
                  ...
                  <div className="w-full md:w-1/2">
                     ...
                     <h3 className="text-2xl font-bold text-slate-800 leading-tight mb-4 group-hover:text-sky-700 transition">
                       {post.title}
                     </h3>
                     <div className="w-12 h-1 bg-slate-200 mb-4"></div>
                     <p className="text-slate-600 text-justify leading-relaxed whitespace-pre-line mb-6">
                       {post.content}
                     </p>
                     <Link href={`/sua-chua`} className="inline-flex items-center gap-2 text-sky-700 font-bold hover:text-orange-600 transition">
