...
import { ArrowLeft, Phone, X, Pencil, Trash2 } from "lucide-react";
...
export default function MuaBanPage() {
  ...
  const loadData = () => {
    ...
    const filtered = allProducts.filter((p: any) => p.category === "mua-ban");
  ...
  const deleteProduct = (e: any, id: number) => {
    ...
    if (confirm("Admin muốn xóa sản phẩm này luôn không?")) {
      const allProducts = JSON.parse(localStorage.getItem("nat_products") || "[]");
  ...
  const editProduct = (e: any, id: number) => {
    e.stopPropagation();
    // Chuyển hướng sang trang Dashboard và mang theo ID sản phẩm
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-10">
      ...
      <div className="container mx-auto p-2 md:p-6 max-w-6xl">
        {products.length === 0 ? (
           <div className="text-center py-20 text-slate-400">Chưa có sản phẩm nào.</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
            {products.map((item) => {
              ...
              return (
                <div key={item.id} onClick={() => openDetail(item)} className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden cursor-pointer hover:border-sky-500 hover:shadow-md transition relative group">
      ...
      {/* POPUP CHI TIẾT (GIỮ NGUYÊN CODE CŨ) */}
      {selectedProduct && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-2 md:p-10 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row relative">
            ...
            <div className="w-full md:w-2/5 p-6 overflow-y-auto bg-white flex flex-col">
              ...
              <div className="flex-1"><h4 className="font-bold text-slate-700 text-sm border-b pb-2 mb-2">MÔ TẢ SẢN PHẨM</h4><p className="text-slate-600 text-sm leading-relaxed whitespace-pre-line">{selectedProduct.desc}</p></div>
              <div className="mt-6 pt-4 border-t sticky bottom-0 bg-white"><a href="tel:0912258461" className="w-full bg-red-600 text-white py-3 rounded-lg font-bold hover:bg-red-700 transition flex items-center justify-center gap-2 animate-bounce"><Phone size={20} /> GỌI MUA NGAY: 0912.258.461</a></div>
            </div>
          </div>
        </div>
      )}
